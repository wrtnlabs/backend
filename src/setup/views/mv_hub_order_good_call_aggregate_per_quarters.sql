...
DROP MATERIALIZED VIEW IF EXISTS hub.mv_hub_order_good_call_aggregate_per_quarters;

CREATE MATERIALIZED VIEW hub.mv_hub_order_good_call_aggregate_per_quarters
AS
SELECT
    OG.id AS hub_order_good_id,
    DATE_TRUNC('quarter', OGC.created_at) AS date,
...
GROUP BY
    OG.id, DATE_TRUNC('quarter', OGC.created_at);

CREATE UNIQUE INDEX uk_mv_hub_order_good_call_aggregate_per_quarters
    ON hub.mv_hub_order_good_call_aggregate_per_quarters ("date", hub_order_good_id);
CREATE INDEX idx_mv_hub_order_good_call_aggregate_per_quarters
    ON hub.mv_hub_order_good_call_aggregate_per_quarters (hub_order_good_id, "date");