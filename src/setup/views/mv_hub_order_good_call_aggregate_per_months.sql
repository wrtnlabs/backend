...
DROP MATERIALIZED VIEW IF EXISTS hub.mv_hub_order_good_call_aggregate_per_months;

CREATE MATERIALIZED VIEW hub.mv_hub_order_good_call_aggregate_per_months
AS
SELECT
    OG.id AS hub_order_good_id,
    DATE_TRUNC('month', OGC.created_at) AS date,
...
GROUP BY
    OG.id, DATE_TRUNC('month', OGC.created_at);

CREATE UNIQUE INDEX uk_mv_hub_order_good_call_aggregate_per_months
    ON hub.mv_hub_order_good_call_aggregate_per_months ("date", hub_order_good_id);
CREATE INDEX ind_mv_hub_order_good_call_aggregate_per_months
    ON hub.mv_hub_order_good_call_aggregate_per_months (hub_order_good_id, "date");