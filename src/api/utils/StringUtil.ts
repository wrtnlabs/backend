...
export namespace StringUtil {
  ...
  export const compareVersion = (x: string, y: string): number => {
    const xValues: number[] = x.replace("v", "").split(".").map(Number);
    const yValues: number[] = y.replace("v", "").split(".").map(Number);
    const length: number = Math.max(xValues.length, yValues.length);
    for (let i: number = 0; i < length; i++) {
      const a: number = xValues[i] ?? 0;
      const b: number = yValues[i] ?? 0;
      if (a > b) return 1;
      if (a < b) return -1;
    }
    return 0;
