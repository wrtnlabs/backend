...
export const test_api_studio_account_secret_value_erase = async (
  pool: ConnectionPool,
): Promise<void> => {
  ...
  await HubApi.functional.studio.customers.accounts.secrets.values.erase(
    pool.customer,
    account.code,
    secret.id,
    value.id,
  ...
  TestValidator.equals("erased")(secret)(read);
};
