...
export const test_api_studio_meta_chat_session_share_erase = async (
  pool: ConnectionPool,
): Promise<void> => {
  ...
  await HubApi.functional.studio.customers.meta.chat.sessions.shares.erase(
    pool.customer,
    share.id,
  );
  await TestValidator.httpError("erased")(404)(() =>
    HubApi.functional.studio.customers.meta.chat.sessions.shares.at(
      pool.customer,
      share.id,
    ),
  );
