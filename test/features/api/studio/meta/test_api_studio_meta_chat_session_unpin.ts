...
export const test_api_studio_meta_chat_session_pin = async (
  pool: ConnectionPool,
): Promise<void> => {
  ...
  await HubApi.functional.studio.customers.meta.chat.sessions.unpin(
    pool.customer,
    session.id,
  );
  ...
  TestValidator.equals("pinned")(!!read.pinned_at)(false);
};
