...
export const test_api_studio_meta_chat_session_chatting_limit_with_talk =
  async (pool: ConnectionPool): Promise<boolean> => {
    ...
    for (let i = 0; i < HubConfiguration.freeTalkingLimit.get() - 1; i++) {
      await consume(i);
    }
    ...
    await realConnector.getDriver().initialize();
    await realConnector.getDriver().talk({
      text: `Test message ${HubConfiguration.freeTalkingLimit.get()}`,
      type: "text",
    });
    ...
    await realConnector
      ...
      .talk({
        type: "text",
        text: `Test message ${HubConfiguration.freeTalkingLimit.get() + 1}`,
      })
      .then(() => {
