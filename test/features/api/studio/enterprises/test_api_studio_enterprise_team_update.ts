...
export const test_api_studio_enterprise_team_update = async (
  pool: ConnectionPool,
): Promise<void> => {
  ...
  const executor = update({ pool, enterprise, owner });
...
const update =
  ...
  async (actor: IHubMember | IStudioEnterpriseEmployee): Promise<void> => {
    const team: IStudioEnterpriseTeam = await create(asset)(asset.owner);

    await login(asset.pool, actor.type === "employee" ? actor.member : actor);

    const input: IStudioEnterpriseTeam.IUpdate = {
      name: RandomGenerator.alphabets(8),
    };
    await HubApi.functional.studio.customers.enterprises.teams.update(
      asset.pool.customer,
...
const create =
  ...
  async (
    actor: IHubMember | IStudioEnterpriseEmployee,
  ): Promise<IStudioEnterpriseTeam> => {
    await login(asset.pool, actor.type === "employee" ? actor.member : actor);
    return generate_random_enterprise_team(asset.pool, asset.enterprise);
  };
