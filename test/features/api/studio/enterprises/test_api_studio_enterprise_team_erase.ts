...
export const test_api_studio_enterprise_team_erase = async (
  pool: ConnectionPool,
): Promise<void> => {
...
const erase =
  ...
  async (actor: IHubMember | IStudioEnterpriseEmployee): Promise<void> => {
    ...
    await HubApi.functional.studio.customers.enterprises.teams.erase(
      asset.pool.customer,
      asset.enterprise.account.code,
      team.id,
    );
    await TestValidator.httpError("erased")(404)(() =>
      HubApi.functional.studio.customers.enterprises.teams.at(
        asset.pool.customer,
        asset.enterprise.account.code,
        team.id,
      ),
