...
export const test_api_hub_order_publish_open = async (
  pool: ConnectionPool,
): Promise<void> => {
  // 관리자와 판매자 준비
  ...
  const time: Date = new Date(Date.now() + 15_000);
  await HubApi.functional.hub.customers.orders.publish.open(
    pool.customer,
    order.id,
    {
  ...
  TestValidator.equals("opened_at")(read.goods.map((g) => g.opened_at))(
