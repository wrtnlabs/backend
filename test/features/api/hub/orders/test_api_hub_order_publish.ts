...
export const test_api_hub_order_publish = async (
  pool: ConnectionPool,
): Promise<void> => {
  // 관리자와 판매자 준비
  ...
  // 모두가 주문 조회 가능
  for (const actor of ["customers", "sellers", "admins"] as const) {
    const reloaded: IHubOrder = await HubApi.functional.hub[actor].orders.at(
      actor === "customers"
        ? pool.customer
        : actor === "sellers"
          ? pool.seller
    ...
    TestValidator.equals("published", TestGlobal.exceptSaleKeys)(order)(
      reloaded,
    );
  }
};
