...
export const test_api_hub_order_good_close = async (
  pool: ConnectionPool,
): Promise<void> => {
  ...
  const closer = async (time: Date | null) => {
    await HubApi.functional.hub.customers.orders.goods.close(
      pool.customer,
      order.id,
      good.id,
      {
        closed_at: time?.toISOString() ?? null,
      },
    );
    ...
    TestValidator.equals("closed_at")(read.closed_at)(
      time?.toISOString() ?? null,
    );
  };

  await TestValidator.httpError("reverse")(422)(() =>
    closer(new Date(Date.now() - MONTH)),
  );
  await closer(new Date(Date.now() + 3 * MONTH));
  await closer(null);
  await closer(new Date(Date.now() + 2 * MONTH));
};
