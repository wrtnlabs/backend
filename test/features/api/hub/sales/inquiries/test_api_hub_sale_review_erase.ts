...
export const test_api_hub_sale_review_erase = async (pool: ConnectionPool) => {
  ...
  await HubApi.functional.hub.customers.sales.reviews.erase(
    pool.customer,
    sale.id,
    review.id,
  );
  ...
  await TestValidator.httpError("erase")(404)(() =>
    HubApi.functional.hub.customers.sales.reviews.at(
      pool.customer,
      sale.id,
      review.id,
    ),
