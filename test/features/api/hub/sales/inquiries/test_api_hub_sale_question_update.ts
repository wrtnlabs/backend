...
export const test_api_hub_sale_question_update = async (
  pool: ConnectionPool,
): Promise<void> => {
  ...
  question.snapshots.push(
    ...(await ArrayUtil.asyncRepeat(3)(async () => {
      const snapshot: IHubSaleQuestion.ISnapshot =
        await HubApi.functional.hub.customers.sales.questions.update(
          pool.customer,
          sale.id,
          question.id,
          {
  ...
  TestValidator.equals("update")(question)(read);
};
